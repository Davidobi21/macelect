...
router.get('/admin', async (req, res) => {
  try {
    const adminId = req.user._id; // Assuming `req.user._id` contains the admin's ObjectId

    if (!mongoose.Types.ObjectId.isValid(adminId)) {
      return res.status(400).json({ error: 'Invalid admin ID' });
    }

    const orders = await Order.find({ adminId }); // Ensure `adminId` is a valid ObjectId
    res.status(200).json({ orders });
  } catch (error) {
    console.error(error);
