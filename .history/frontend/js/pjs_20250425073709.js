document.addEventListener("DOMContentLoaded", async function () {
  ...
  function attachCardClickListeners() {
    productCards.forEach(card => {
      card.addEventListener("click", () => {
        ...
        const mainImage = card.querySelector(".main-img").src;
        const hoverImage = card.querySelector(".hover-img")?.src || mainImage;

        // Construct the URL with query parameters
        const productPageUrl = `./productpage.html?name=${encodeURIComponent(name)}&category=${encodeURIComponent(category)}&price=${encodeURIComponent(price)}&mainImage=${encodeURIComponent(mainImage)}&hoverImage=${encodeURIComponent(hoverImage)}`;
  ...
  try {
    ...
    // Attach click listeners to product cards for navigation
