document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateCartCount() {
      ...
      $('#addToCartButton').on('click', function () {
        const product = {
          name: $('h1').text().trim(),
          price: $('.product-price').text().replace('₦', '').trim(),
          color: $('.color-options .selected').data('color') || '', // assuming selected color has class 'selected'
          size: $('.size-dropdown').val(),
          quantity: parseInt($('input[type=number]').val(), 10),
