document.addEventListener("DOMContentLoaded", async function () {
    ...
    function filterProducts() {
      ...
      productCards.forEach(card => {
        ...
        const category = card.dataset.category.toLowerCase(); // Ensure category is in lowercase for comparison
        const price = parseFloat(card.dataset.price);
  
        const matchesSearch = name.includes(searchText);
        const matchesCategoryBtn = selectedCategory === "All" || category === selectedCategory.toLowerCase();
        const matchesSidebarCat = checkedCategories.length === 0 || checkedCategories.map(c => c.toLowerCase()).includes(category);
